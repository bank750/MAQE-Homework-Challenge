<template>
    <div class="page-wrapper">
        <div class="card-wrapper">
            <h2 class="header-text">MAQE Forum</h2>
            <h3 class="timezone-text">Your current timezone is: {{ currentTimezone }}</h3>
            <template v-for="(item) in dataChunk">
                <card-h :data="item" />
            </template>     
        </div>
    </div>
...
<script setup lang="ts">
    ...
    import postServive from '@/services/post/post.servive';
    import { leftJoin } from '@/utils/helper';
    import axios from 'axios';
    import { onBeforeUnmount, onMounted, ref } from 'vue';
    import moment from 'moment-timezone';
    import { ICardItem } from '@/interface/card.interface';

    const currentTimezone = moment.tz.guess();
    const cancelTokenAuthor = ref(axios.CancelToken.source())
	const cancelTokenPost = ref(axios.CancelToken.source())
    const dataList = ref<any[]>([])
    const loadData = async() => {
        cancelTokenAuthor.value.cancel()
		cancelTokenPost.value.cancel()
		cancelTokenAuthor.value = axios.CancelToken.source()
		cancelTokenPost.value = axios.CancelToken.source()
        try {
			const res: any[] = await Promise.all([
            postServive.getAllPosts({ cancelToken: cancelTokenPost.value.token }),
            authorService.getAllAuthors({ cancelToken: cancelTokenAuthor.value.token }),
			])
            if (res) {
                const mergedArray = leftJoin({ array1: res[0], array2: res[1], key1: 'author_id', key2: 'id'})                
                dataList.value = mergedArray                
            }
		} catch (e: any) {
			if (axios.isCancel(e)) return
            console.error(e.response?.data?.message || e.message || 'something went wrong');
		}
    }
    dataList.value = [
    {
    ...
    const dataChunk = ref<ICardItem[]>([])
    const chunkSize = ref<number>(5)
    const  currentChunk = ref<number>(1)

    dataChunk.value = dataList.value.slice(0, 5)
    const loadMoreItems = () => {
      const startIndex = (currentChunk.value - 1) * chunkSize.value;
      const endIndex = Math.min(currentChunk.value * chunkSize.value, dataList.value.length);
      dataChunk.value = dataChunk.value.concat(dataList.value.slice(startIndex, endIndex));
      currentChunk.value++;
    }
    const handleScroll = () => {        
        const bottomOfPage = document.body.scrollHeight - window.innerHeight;
        if (window.scrollY + 10 >= bottomOfPage) {
        if (dataChunk.value.length < dataList.value.length) {
            loadMoreItems()            
        }
        }
    }
        ...
        .card-wrapper {
            ...
            .card-h {
                &:nth-child(2n) {
                    ...
                    :deep(.card-header) {
                        border-bottom: 2px solid rgb(190, 216, 233) !important;
                    }
                }
                &:nth-child(2n+1) {
